...
class MLPTagCnn():
    ...
    def __init__(self, sequence_length, vocab_size, tagVocab_size, embedding_size, tagEmb_size, window_size, num_filters, hidden_size, margin,
                l2_reg_lambda=0.0):
        ...
        self.total_embedding_size = embedding_size + tagEmb_size
        ...
        with tf.variable_scope("Tag_Embedding"):
            self.tag_embedding = tf.get_variable("tag_embedding", shape=[tagVocab_size, tagEmb_size])
        ...
        input_q = tf.concat([input_q, tag_q], -1)
        input_pos = tf.concat([input_pos, tag_pos], -1)
        input_neg = tf.concat([input_neg, tag_neg], -1)

        with tf.variable_scope("CNN"):

            self.kernels = []
            for i, filter_size in enumerate(self.win_size):
                filter_shape = [filter_size, self.total_embedding_size, 1, self.num_filters]
